...
class EnhancedWebSpider:
    ...
    def process_url(self, url, depth):
        if not self.should_visit(url) or (self.max_depth > 0 and depth > self.max_depth) or len(self.visited_urls) >= self.max_pages:
            return []
        ...
        if self.max_parse_size > 0 and len(page_content) > self.max_parse_size:
            logger.debug(f"Skipping {url} due to response size {len(page_content)} exceeding max_parse_size")
            return []
        ...
        new_urls = []
        for link in links:
            if self.should_visit(link) and len(self.visited_urls) < self.max_pages:
