...
class EnhancedWebSpider:
    ...
    def crawl(self):
        ...
        while self.to_visit and self.should_continue():
            url, depth = self.to_visit.popleft()  # Dequeue from the left for BFS
    ...
    def process_url(self, url, depth):
        """
        Processes the given URL by fetching its content and extracting links.
        """
        if not self.should_visit(url) or len(self.visited_urls) >= self.max_pages:
            return []
