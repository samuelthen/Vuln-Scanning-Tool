...
class EnhancedWebSpider:
    ...
    def extract_domain_and_add_to_scope(self, url):
        ...
        domain = parsed_url.netloc.lower()  # Extract and normalize the domain
        print(domain)
    ...
    def is_domain_in_scope(self, url):
        ...
        # Check if the domain is in the list of domains in scope
    ...
    def should_visit(self, url):
        ...
        return (
            ...
            self.is_domain_in_scope(url) and
            (not self.respect_robots_txt or self.is_allowed_by_robots(url)) 
        )
    ...
    def fetch_page(self, url):
        ...
        for attempt in range(self.retries):
            try:
                ...
                # Send the request
                response = self.session.send(prepared_req) if self.session else requests.send(prepared_req)
                response.raise_for_status()
                
                # Store the request and response in the mapping
                self.url_request_response_map[url] = (prepared_req, response)
...
if __name__ == "__main__":
    ...
    spider = EnhancedWebSpider(url, max_pages=10, max_depth=3, max_threads=10)
    spider.crawl()

    # After crawling, you can access the URL-to-Request/Response mapping
    for url, (request, response) in spider.url_request_response_map.items():
        print(f"URL: {url}")
