...
class EnhancedWebSpider:
    ...
    def crawl(self):
        ...
        # Add this line to parse sitemap.xml
        if self.parse_sitemap_xml:
            self.parse_sitemap_xml()
        ...
        logger.debug(f"Crawled {len(self.visited_urls)} pages.")
        self.save_results()
    ...
    def process_url(self, url, depth):
        ...
        # Add these lines to parse metadata
        if self.parse_svn_metadata:
            self.parse_svn_metadata(normalized_url)
        
        if self.parse_git_metadata:
            self.parse_git_metadata(normalized_url)
        
        if self.parse_ds_store_files:
            self.parse_ds_store_files(normalized_url)
    ...
    def parse_sitemap_xml(self):
        """
        Parses the sitemap.xml file to extract URLs.
        """
        sitemap_url = urljoin(self.base_url, "/sitemap.xml")
        try:
            response = self.session.get(sitemap_url) if self.session else requests.get(sitemap_url)
            if response.status_code == 200:
                root = ET.fromstring(response.content)
                namespace = {'ns': 'http://www.sitemaps.org/schemas/sitemap/0.9'}
                for url in root.findall('.//ns:loc', namespace):
                    self.to_visit.append((url.text, 0))
                logger.debug(f"Added {len(self.to_visit)} URLs from sitemap.xml")
            else:
                logger.debug("No sitemap.xml found or inaccessible.")
        except requests.RequestException as e:
            logger.error(f"Failed to fetch sitemap.xml: {e}")
        except ET.ParseError as e:
            logger.error(f"Failed to parse sitemap.xml: {e}")

    def parse_svn_metadata(self, base_url):
        """
        Parses SVN metadata to extract potential URLs.
        """
        svn_urls = [
            urljoin(base_url, '.svn/entries'),
            urljoin(base_url, '.svn/wc.db')
        ]
        for url in svn_urls:
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Extract URLs from SVN entries or wc.db
                    extracted_urls = re.findall(r'(https?://\S+)', content)
                    for extracted_url in extracted_urls:
                        self.to_visit.append((extracted_url, 0))
                    logger.debug(f"Extracted {len(extracted_urls)} URLs from SVN metadata")
            except requests.RequestException as e:
                logger.error(f"Failed to fetch SVN metadata from {url}: {e}")

    def parse_git_metadata(self, base_url):
        """
        Parses Git metadata to extract potential URLs.
        """
        git_urls = [
            urljoin(base_url, '.git/config'),
            urljoin(base_url, '.git/HEAD')
        ]
        for url in git_urls:
            try:
                response = self.session.get(url) if self.session else requests.get(url)
                if response.status_code == 200:
                    content = response.text
                    # Extract URLs from Git config or HEAD
                    extracted_urls = re.findall(r'(https?://\S+)', content)
                    for extracted_url in extracted_urls:
                        self.to_visit.append((extracted_url, 0))
                    logger.debug(f"Extracted {len(extracted_urls)} URLs from Git metadata")
            except requests.RequestException as e:
                logger.error(f"Failed to fetch Git metadata from {url}: {e}")

    def parse_ds_store_files(self, base_url):
        """
        Parses .DS_Store files to extract potential filenames and directories.
        """
        ds_store_url = urljoin(base_url, '.DS_Store')
        try:
            response = self.session.get(ds_store_url) if self.session else requests.get(ds_store_url)
            if response.status_code == 200:
                ds_store = DSStore.open(BytesIO(response.content))
                for entry in ds_store:
                    if entry.filename:
                        new_url = urljoin(base_url, entry.filename)
                        self.to_visit.append((new_url, 0))
                logger.debug(f"Extracted {len(ds_store)} entries from .DS_Store")
        except requests.RequestException as e:
            logger.error(f"Failed to fetch .DS_Store from {ds_store_url}: {e}")
        except Exception as e:
            logger.error(f"Failed to parse .DS_Store: {e}")

    def save_results(self):
        """
        Saves the results of the crawl to the output file.
        """
        data = {
