...
class PassiveScanner:
    ...
    def run_scan(self, request: Request, response: Response) -> Dict[str, List[Alert]]:
        results = {}
        logger.info(f"Scanning: {request.url}")

        with ThreadPoolExecutor(max_workers=len(self.scan_rules)) as executor:
            future_to_scan_rule = {
                executor.submit(scan_rule.check_risk, request, response): scan_rule
                for scan_rule in self.scan_rules
            }

            for future in as_completed(future_to_scan_rule):
                scan_rule = future_to_scan_rule[future]
                try:
                    result = future.result()
                    results[str(scan_rule)] = result
                except Exception as e:
                    logger.error(f"Error in {scan_rule}: {e}")
